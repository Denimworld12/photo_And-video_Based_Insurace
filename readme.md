...
| `GEMINI_API_KEY` | No | Without it the "plain-language summary" button reports that it is unavailable |
...
| Browser console shows a CORS error | Add the frontend's origin to `ALLOWED_ORIGINS` or `FRONTEND_URL` in `backend/.env`. |
